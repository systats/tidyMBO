...
```{r}
#devtools::install_github("systats/tidy.mbo", force = T)
pacman::p_load(tidyverse, dplyr, purrr, rsample, data.table, magrittr, tidyTX, keras, mlrMBO, tidy.mbo, ggthemes, Smisc, randomForest, parallelMap, emoa)
#keras::install_keras()
set.seed(2018)
...
params <- makeParamSet(
    ...
    makeDiscreteParam("arch", values = c("fasttext", "lstm", "bilstm", "mlp"))
    ### LSTM only
...
mode1 <- list(maxlen = 30)%>% 
  run_keras_steps(data = dt, target = "party_id", text = "text_lemma", reconstruct = T)
