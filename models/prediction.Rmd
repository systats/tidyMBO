...
pacman::p_load(dplyr, ggplot2, googlesheets, openxlsx, stringr, rvest, dplyr, ggplot2, keras, mlrMBO, tidyMBO, ggthemes, Smisc, randomForest, parallelMap, emoa, DiceKriging, magrittr, h2o, ParamHelpers)
...
twitter_dict <- list(
  ...
  "hebrew" = " jew ",
  "heeb" = " jew swearword ",
  "libtards" = " liberal swearword ",
  "n.gg.." = " race swearword ",
  "kike" = " jew swearword ",
  "left" = "liberal",
  "femnazi." = " woman swearword ",
...
final_glove <- results_glove$params %>%
  ...
  run_mbo_steps(reconstruct = T)

#mat <- table(dnn$perform, dnn$data$test$alt_type)
mat <- table(gbm$perform, gbm$data$test$alt_type)
...
params_gbm <- makeParamSet(
    makeIntegerParam("max_vocab", lower = 1500, upper = 2500),
    makeIntegerParam("ntrees", lower = 20, upper = 150),
    makeIntegerParam("max_depth", lower = 2, upper = 10), # before stopping
    makeNumericParam("learn_rate", lower = .1, upper = .9), # need for splitting
    makeNumericParam("sample_rate", lower = .5, upper = .9), # use a 
    makeNumericParam("stop_tol", lower = .001, upper = .1),
    makeIntegerParam("stop_round", lower = 1, upper = 3), # use a
    makeIntegerParam("nbins", lower = 10, upper = 20) # need for splitting
  )
...
```{r, eval = F}
# save(results_gbm, file = "shiny_mbo/results/gbm_text_word.Rdata")
perform <- results_gbm
perform$params %>%
  arrange(desc(accuracy)) %>%
  glimpse()
...
table(final_model$perform$predict, final$test$alt_type)
...
params_dnn <- makeParamSet(
    ...
    makeIntegerParam("epochs", lower = 4, upper = 8), # need for splitting
    # makeNumericParam("rho", lower = .95, upper = .9999999), # use a random 70% of the rows 
...
```{r, eval = F}
#save(results_gbm, file = "results/gbm_xxxx.Rdata")
perform <- results_dnn
perform %>%
  arrange(desc(accuracy)) %>%
  glimpse()
...
table(final_model1$perform$predict, final$test$alt_type)
```
